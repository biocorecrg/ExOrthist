...
ExOrthist main module infers exon orthopairs and exon orthogroups within the gene orthogroups provided as input (e.g. generated by [Orthofinder](https://github.com/davidemms/OrthoFinder), [Broccoli](https://github.com/rderelle/Broccoli) or similar tools) for all the species for which annotation (gtf) and genomic (fasta) files are provided. In ordert\ to fine-tune the orthology calls, evolutionary distance ranges between each pair of species (short, medium long) also need to be specified [see [Algorithm](#algorithm) for a detailed explanation of the `main.nf` logic].    
...
#### Facultative outputs
ExOrthist is also able to consider non-annotated exons in the orthology inference, which can be added via the **--extraexons** argument [[see Inputs](#inputs)]. If non-annotated exons are added, ExOrthist also generates the following files:  
**output_main/\${species}:** 
...
* **Not-annotated** exons (given as facultative input to main.nf, **--extraexons** argument, [[see Inputs](#Inputs)]) are represented as white boxes.  
...
* Exons in the query gene taken from the **--relevant_exs** argument [[see Inputs](#Inputs)] (and their orthologs) are highlighted with different colors.  
