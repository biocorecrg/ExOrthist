...
* **filtered\_best\_scored\_exon\_matches\_by\_gene-NoOverlap.txt**: it contains the same information as filtered_best_scored_EX_matches_by_targetgene.tab, but exclusively for a representative exon in each group of overlapping exons (i.e. different versions of the same exon). The representative exon is the one with the higher number of matches across species.    
...
ExOrthist also allows the introduction of exon pairs considered bona fide orthology relationships into the exon orthology inference. Such exons can be specified with the **--bonafide_pairs** flag [[see Inputs](#inputs)].  
...
`get_liftovers.pl` parses exons at the CDS level by default (as ExOrthist), but it can work with mRNA exons if the option is **--type exon** is provided. Furthermore, the script can require matches to have at least one cannonical dinucleotide using the **--canonical_ss** flag. Additional information is provided in the help message of `get_liftovers.pl`.

### D. Exon clustering 
After deriving exon orthologous relationships between each pair of species, ExOrthist works on the combined information to infer exon orthology groups.  
For each gene cluster, ExOrthist builds a directed graph (through the R package **igraph**) with exons as nodes and their pairwise orthologous relationships represented as edges. The ExOrthist then applies an edge-betweenness algorithm to select the optimal graph topology. Even if the directionality of the graph is not considered by the edge-betweeness algorithm, the reciprocality of the matches is anyway represented by the number of edges. The exon communities (or clusters) identified in the optimal topology are returned as exon orthogroups. ExOrhthis also computes a Membership Score (MS) for each exon, reflecting its degree of similarity to all the other exons in its belonging cluster. The MS is defined as:  
...
* Exons in the query gene taken from the **--relevant_exs** argument [[see Inputs](#inputs-1)] (and their orthologs) are highlighted with different colors.  
...
This is an example output text of the summary statistics for the pairwise comparisons:
