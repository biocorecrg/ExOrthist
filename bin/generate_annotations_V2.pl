...
if (!defined $genome_file || !defined $gtf_file || !defined $species_id || defined $help){
    die "\nUsage: generate_annotations.pl -GTF path_to_gtfs/ -G path_to_genomes/ -sp Sp1 [-EX_DB path_to_EXONS_DB/ -vastdb REF1,REF2]
	...
	COMPULSORY
     ...
     -sp Sp1           Species id.
	OPTIONAL
     -vastdb r1        VASTDB reference file (must match species list in -sp)
...
my (%chr1, %chr2);
if ($gtf_file =~ /.gz$/) {
    open(INFILEONE, "gunzip -c $gtf_file |") || die "t open pipe to $gtf_file";
}
else {
