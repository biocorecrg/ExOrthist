...
input_orthopairs <- args[1] #orthopairs with best reciprocal information
output_orthogroup <- args[2]
output_unclustered_exons <- args[3]
...
for (my_gene_OG in all_gene_OG) {
  #subset to only a specific gene orthogroup
  my_table = subset(input_table, clusterID==my_gene_OG)[,c("ID1", "ID2", "Best_reciprocal")]
  ######## Generate graph #############
  ...
  write.table(single_exon_clusters_tosave_df, file=output_unclustered_exons, append=TRUE, quote=FALSE, sep="\t", row.names = FALSE, col.names=FALSE)
  ...
  #save to file
  write.table(my_final_clusters, file=output_orthogroup, append=TRUE, quote=FALSE, sep="\t", row.names = FALSE, col.names=FALSE)
}
