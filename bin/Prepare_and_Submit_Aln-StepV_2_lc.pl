...
### Help
if (!defined ($f_gene_cluster) || !defined($outfolder)|| !defined($dir) || !defined($species_array) || defined ($help)){
    die "
Usage: Prepare_and_Submit_Aln-StepV_2.pl -sp Sp1,Sp2 --gene_cluster FILE [options]
...
if (defined $verbose){
    print "\nrm -r $outfolder/$project_dir\n" if (defined ($clean) && -e "$outfolder/$project_dir");
    print "mkdir $outfolder/$project_dir\n" unless (-e "$outfolder/$project_dir");
    print "mkdir $outfolder/$project_dir/GENE_CLUSTERS\n" unless (-e "$outfolder/$project_dir/GENE_CLUSTERS");
}
my %N_parts; # stores the number of splits for each file of clusters
my $gcl=0;
system "rm -r $outfolder/$project_dir" if (defined ($clean) && -e "$outfolder/$project_dir");
system "mkdir -p $outfolder/$project_dir" unless (-e "$outfolder/$project_dir");
if (-e "$outfolder/$project_dir/GENE_CLUSTERS"){ 
	$gcl=1; 
	my $parts=`ls $outfolder/$project_dir/GENE_CLUSTERS/*part* | wc -l`;
	$N_parts{$f_gene_cluster}=$parts;

} else { system "mkdir $outfolder/$project_dir/GENE_CLUSTERS"; }
...
for my $i (0..$#species){
	...
	if ($i < $j){ # only does Sp1_Sp2 (not Sp2_Sp1)
	    ...
	    if (defined ($verbose)){
		print "\nrm -r $outfolder/$project_dir/$pair_folder\n" if $clean && (-e "$outfolder/$project_dir/$pair_folder"); #Forces full deletion of data
		print "\nmkdir $outfolder/$project_dir/$pair_folder\n" unless (-e "$outfolder/$project_dir/$pair_folder");
	    }
	    system "rm -rf $outfolder/$project_dir/$pair_folder" if $clean && (-e "$outfolder/$project_dir/$pair_folder"); #Forces full deletion of data
	    system "mkdir $outfolder/$project_dir/$pair_folder" unless (-e "$outfolder/$project_dir/$pair_folder");
