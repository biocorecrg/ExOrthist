...
if (!defined ($dir) || !defined ($bin) ||  !defined($species_array) || defined ($help)){
    die "
Usage: Run_join_all_scores.pl -sp Sp1,Sp2,Sp3 --dir PATH [options]
...
OPTIONS
    ...
    --bin PATH                Root directory where bin files are located
...
for my $i (0..$#species){
	...
	if ($i < $j){ # only does Sp1_Sp2 (not Sp2_Sp1)
	    ...
	    $in=$pair_folder."/Score_all_exons.txt";
	    $of=$pair_folder."/Best_scores_pair_exons.txt";
